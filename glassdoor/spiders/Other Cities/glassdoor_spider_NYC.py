...
class glassdoor_spider(scrapy.Spider):
    ...
    start_urls = ['https://www.glassdoor.com/Job/new-york-data-scientist-jobs-SRCH_IL.0,8_IC1132348_KO9,23.htm']

    def parse(self, response):
        results_url_list = ['https://www.glassdoor.com/Job/new-york-data-scientist-jobs-SRCH_IL.0,8_IC1132348_KO9,23.htm']
        for i in range(2, 34):
            results_url_list.append('https://www.glassdoor.com/Job/new-york-data-scientist-jobs-SRCH_IL.0,8_IC1132348_KO9,23_IP'+str(i)+'.htm')
    ...
    def parse_listing(self, response):
        ...
        #title = response.xpath('//h2[@class="noMargTop margBotXs strong"]/text()').extract_first()
        #company = response.xpath('//div[@class="cell padTop empHeader"]/span/text()').extract_first()
        #location = response.xpath('//div[@class="cell padTop empHeader"]/span/text()').extract_first()
        #salary_est = response.xpath('.//div[@class="padTopSm"]//text()').extract_first()
